...
if ( ! function_exists( 'ehri_training_render_source_metadata_fields' ) ) {
	...
	function ehri_training_render_source_metadata_fields( $term = null ) {
		...
		<tr class="form-field">
			<th scope="row"><label for="term_unit"><?php esc_html_e( 'Unit' ); ?></label></th>
			<td>
				<select name="term_unit" id="term_unit">
					<option value=""><?php esc_html_e( 'Select Unit' ); ?></option>
					<?php
					$units = get_terms(
						array(
							'taxonomy' => 'unit',
				...
				<p class="description"><?php esc_html_e( 'Select the unit this source belongs to.' ); ?></p>
			</td>
		</tr>
	...
	function ehri_training_save_source_metadata( $term_id ) {
		...
		if ( isset( $_POST['term_unit'] ) ) {
			update_term_meta(
				$term_id,
				'term_unit',
				sanitize_text_field( wp_unslash( $_POST['term_unit'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_location'] ) ) {
			update_term_meta(
				$term_id,
				'term_location',
				sanitize_text_field( wp_unslash( $_POST['term_location'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_source_file'] ) ) {
			update_term_meta(
				$term_id,
				'term_source_file',
				intval( wp_unslash( $_POST['term_source_file'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_source_name'] ) ) {
			update_term_meta(
				$term_id,
				'term_source_name',
				sanitize_text_field( wp_unslash( $_POST['term_source_name'] ) )
			);
		}
		...
		if ( isset( $_POST['term_source_url'] ) ) {
			update_term_meta(
				$term_id,
				'term_source_url',
				esc_url_raw( wp_unslash( $_POST['term_source_url'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_collection_name'] ) ) {
			update_term_meta(
				...
				sanitize_text_field( wp_unslash( $_POST['term_collection_name'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_collection_url'] ) ) {
			update_term_meta(
				$term_id,
				'term_collection_url',
				esc_url_raw( wp_unslash( $_POST['term_collection_url'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_translation_file'] ) ) {
			update_term_meta(
				...
				intval( wp_unslash( $_POST['term_translation_file'] ) )
			);
		} else {
		...
		if ( isset( $_POST['term_transcription_file'] ) ) {
			update_term_meta(
				...
				intval( wp_unslash( $_POST['term_transcription_file'] ) )
			);
		} else {
...
if ( ! function_exists( 'ehri_training_enqueue_source_metadata_scripts' ) ) {
	/**
	 * Enqueue scripts for source metadata functionality.
	 */
...
if ( ! function_exists( 'ehri_training_init_source_metadata' ) ) {
	/**
	 * Initialize source metadata functionality.
	 */
